import express from "express";
import axios from "axios";
import jwt from "jsonwebtoken";
import User from "../models/User.js";

const router = express.Router();

router.get("/github", (req, res) => {
  const callback = req.query.callback;
  if (!callback) return res.status(400).send("Missing callback URL");

  // Save callback URL in a temporary state
  const state = encodeURIComponent(callback);

  const githubURL = 
    `https://github.com/login/oauth/authorize` +
    `?client_id=${process.env.GITHUB_CLIENT_ID}` +
    `&redirect_uri=${process.env.GITHUB_REDIRECT_URI}` +
    `&scope=read:user` +
    `&state=${state}`;

  res.redirect(githubURL);
});



router.get("/github/callback", async (req, res) => {
  const { code, state } = req.query;

  if (!state) return res.status(400).send("Missing state/callback");

  // Restore CLI callback URL
  const callback = decodeURIComponent(state);

  try {
    // Exchange code for token
    const tokenRes = await axios.post(
      "https://github.com/login/oauth/access_token",
      {
        client_id: process.env.GITHUB_CLIENT_ID,
        client_secret: process.env.GITHUB_CLIENT_SECRET,
        code,
      },
      {
        headers: { Accept: "application/json" }
      }
    );

    const accessToken = tokenRes.data.access_token;

    // Fetch profile
    const profileRes = await axios.get("https://api.github.com/user", {
      headers: { Authorization: `Bearer ${accessToken}` }
    });

    const gh = profileRes.data;

    // Upsert user
    let user = await User.findOne({ githubId: gh.id });
    if (!user) {
      user = await User.create({
        githubId: gh.id,
        username: gh.login,
        avatar: gh.avatar_url,
      });
    }

    // Create JWT
    const jwtToken = jwt.sign(
      { id: user._id, username: user.username },
      process.env.JWT_SECRET,
      { expiresIn: "7d" }
    );

    // Redirect back to CLI
    return res.redirect(`${callback}?token=${jwtToken}`);

  } catch (err) {
    console.error(err);
    return res.status(500).send("GitHub login failed");
  }
});


export default router;

